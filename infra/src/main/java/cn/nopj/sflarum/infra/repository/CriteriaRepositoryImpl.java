package cn.nopj.sflarum.infra.repository;
import cn.nopj.sflarum.domian.entity.Criteria;
import cn.nopj.sflarum.domian.repository.CriteriaRepository;
import cn.nopj.sflarum.infra.mapper.CriteriaMapper;
import cn.nopj.sflarum.infra.po.CriteriaPo;
import lombok.RequiredArgsConstructor;
import org.springframework.beans.BeanUtils;
import org.springframework.stereotype.Repository;
import java.util.ArrayList;
import java.util.List;
@Repository
@RequiredArgsConstructor
public class CriteriaRepositoryImpl implements CriteriaRepository {
    private final CriteriaMapper mapper;
    @Override
    public Criteria findById(Long id) {
        CriteriaPo po = mapper.findById(id);
        if (po == null) {
            return null;
        }
        Criteria entity = new Criteria();
        BeanUtils.copyProperties(po, entity);
        return entity;
    }
    @Override
    public List<Criteria> findAll() {
        List<CriteriaPo> list = mapper.findAll();
        List<Criteria> result = new ArrayList<>();
        if (list == null) {
            return result;
        }
        for (CriteriaPo po : list) {
            Criteria entity = new Criteria();
            BeanUtils.copyProperties(po, entity);
            result.add(entity);
        }
        return result;
    }
    @Override
    public boolean create(Criteria entity) {
        CriteriaPo po = new CriteriaPo();
        BeanUtils.copyProperties(entity, po);
        return mapper.insert(po) > 0;
    }
    @Override
    public boolean update(Criteria entity) {
        CriteriaPo po = new CriteriaPo();
        BeanUtils.copyProperties(entity, po);
        return mapper.update(po) > 0;
    }
    @Override
    public boolean delete(Long id) {
        return mapper.deleteById(id) > 0;
    }
}